...
#pragma ident "@(#)$RCSfile: if_mfa_data.c,v $ $Revision: 1.1.7.5 $ (DEC) $Date: 1996/02/28 18:03:04 $"
...
/*
 * Number of cluster mbufs we'll preallcoate for the PARAM and USER 
 * commands. We really only need 2-3 at most.
 */
#define	MAX_CMD_MBUFS	4
...
struct mfa_softc {
	...
	/* see if_mfa.c for an explanation of why we have two sets of counters
	 * These counter blocks must start on a quadword boundary
	 */
	mfactrblk_t 	cur_xctrs;		/* current copy of counters */
	mfactrblk_t 	base_xctrs;		/* base copy of counters */

	simple_lock_data_t lk_mfa_softc;	/* SMP lock for mfa_softc */
	mfaparam_t 	xparams;		/* saved copy of param block */
	mfastatus_t	xstat; 			/* saved copy of STATUS cmd */
	mfacmd_ent_t	*prevcmd;		/* ring ptr to previous cmd */
	u_int		t_req;		/* FDDI settable parameters */
	u_int		rtoken;
	u_int		tvx;
	u_int		lem; 
	u_int		ring_purger;
	struct	mbuf 	*cmd_mbufs;		/* stack of prealloc'd mbufs */
};
