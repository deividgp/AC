 ...
 * HISTORY
 */
#pragma ident "@(#)$RCSfile: sec_data.c,v $ $Revision: 1.1.11.5 $ (DEC) $Date: 1995/09/20 18:57:53 $"
	...
	long *retval;
{
	...
	switch (uap->swfunc) {
	  ...
	  case SEC_SWITCH_OFF:
		do_audit = 1;
		if (!(error = suser(u.u_cred, &u.u_acflag))) {
			/* Maybe someday add a once-only latch here */
		}
		break;
	  default:
		do_audit = 1;
...
/* Mimic proplist_syscall, but use the user-supplied cred.
   This needs a better home.  Unfortunately, this is where
   the security system call lives for now.
*/
extern fgetproplist();
extern fsetproplist();
extern fdelproplist();

int eproplist(p, args, retval)
struct proc *p;
void *args;
long *retval;
{
	...
	struct ucred cred;
	union syscall_args {
		struct fsetargs fsetargs;
		struct fgetargs fgetargs;
		struct fdelargs fdelargs;
	} syscall_args_un;
	int error = 0;

#if SEC_BASE
	if (!privileged(SEC_SETPROCIDENT, EPERM)) /* XXX: this code is intended for
						     use by the sideband proplistd;
						     pick the correct privilege.
					     	  */
#else
	...
	/* get cred against which to attempt proplist operation */
	if (!error)
		error = copyin(uap->credp, &cred, sizeof(struct ucred));


	/* courtesy of proplist_syscall, but only allow variants which
	   work against file descriptors

           - auditing is done with fxxxproplist()
	*/
	if (!error) switch(uap->index) {
	      case (FGET_PROPLIST_SYSCALL) :
		      error = copyin(uap->buf, &syscall_args_un.fgetargs,
				     sizeof(struct fgetargs));
		      if (!error) {
			      error = fgetproplist(&syscall_args_un.fgetargs,
						   retval, &cred);
		      }
		      break;

	      case (FSET_PROPLIST_SYSCALL) :
		      error = copyin(uap->buf, &syscall_args_un.fsetargs,
				     sizeof(struct fsetargs));
		      if (!error) {
			      error = fsetproplist(&syscall_args_un.fsetargs,
						   retval, &cred);
		      }
		      break;

	      case (FDEL_PROPLIST_SYSCALL) :
		      error = copyin(uap->buf, &syscall_args_un.fdelargs,
				     sizeof(struct fdelargs));
		      if (!error) {
			      error = fdelproplist(&syscall_args_un.fdelargs,
						   retval, &cred);
		      }
		      break;
	      ...
	      default:
		      error = EINVAL;
		      AUDIT_CALL2 ( SYS_proplist_syscall, error, &uap->index, *retval,
		          AUD_VHPR, "a000000" );
		      break;
	}
