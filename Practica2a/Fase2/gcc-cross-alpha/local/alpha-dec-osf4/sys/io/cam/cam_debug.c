 ...
 * HISTORY
 */
#pragma ident "@(#)$RCSfile: cam_debug.c,v $ $Revision: 1.1.16.3 $ (DEC) $Date: 1995/07/05 15:12:38 $"
...
int report_format;
{
	struct CAM_FunctionTable *cst = cam_FunctionTable;
	I32 entrys;

	for (entrys = 0; entrys < cam_FunctionEntrys; entrys++, cst++) {
...
int report_format;
{
	...
	for (entrys = 0; entrys < cam_StatusEntrys; entrys++, cst++) {
...
int report_format;
{
	...
	for (entrys = 0; entrys < scsi_StatusEntrys; entrys++, cst++) {
...
register CCB_PATHINQ *ccb;
{
    ...
    dp ("    The HBA engine count ............ cam_hba_eng_cnt: 0x%x\n",
							ccb->cam_hba_eng_cnt);
    dp ("    Number Targets HBA supports ...... cam_vu_targets: 0x%x\n",
							ccb->cam_vu_targets);
    dp ("    Number Luns HBA supports ............. cam_vu_lun: 0x%x\n",
							ccb->cam_vu_lun);
...
caddr_t
cdbg_SystemStatus (errnum)
register int errnum;
{
	if (errnum < system_ErrorEntrys) {
		return (system_ErrorTable[errnum].errno_msg);
	} else {
		return ("Unknown");
	}
}
