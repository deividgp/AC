 ...
 */
/*
 * @(#)$RCSfile: if_ottoreg.h,v $ $Revision: 1.1.8.5 $ (DEC) $Date: 1995/10/26 21:17:55 $
 */
/*
...
#define OTTO_WRITE_HIGH(addr,flip,mask)	\
	OTTO_PIOW ((volatile otto_uword32 *)(((char *)(addr))+OTTO_RAMA-OTTO_RAM), (((flip) >> 16) | (mask)))
...
#define OTTO_WRITE_LOW(addr,flip,mask)	\
	OTTO_PIOW ((volatile otto_uword32 *)(((char *)(addr))+OTTO_RAMB-OTTO_RAM), ((flip) | ((mask) << 16)))
...
#define OTTO_CSR_LINKOKLED	(1 << 6)	/* 1 => left LED lit */
#define OTTO_CSR_MODOKLED	(1 << 7)	/* 1 => right LED lit */
...
#define OTTO_PHYS_TO_RPTBASE(a,isoppo)	OTTO_PHYS_TO_BUS(a,isoppo)
	/* converts a physical byte address to the format used by the 
	   report ring pointer register
         */
#define OTTO_RPTBASE_TO_PHYS(a,isoppo)	OTTO_BUS_TO_PHYS(a,isoppo)
	/* converts from the format used by the report ring pointer register
	   to a physical byte address
         */

#define OTTO_PHYS_TO_BUS(phys,isoppo) (isoppo?(phys):OTTO_PHYS_TO_TC(phys))
#define OTTO_BUS_TO_PHYS(busa,isoppo) (isoppo?(busa):OTTO_TC_TO_PHYS(busa))
#define OTTO_PHYSW_TO_BUS(physw,isoppo) (isoppo?(physw):OTTO_PHYSW_TO_TC(physw))
#define OTTO_BUS_TO_PHYSW(busa,isoppo) (isoppo?(busa):OTTO_TC_TO_PHYSW(busa))
...
#define OTTO_REP_SEQ_CLP	(1 << 29)	/* cell loss priority */
...
#define OTTO_MAKE_TXPKT0(crc,vc,frag) ((crc) | ((vc) << 16) | ((frag)<<1))
...
#define OTTO_MAKE_FRAG1(addr,isoppo)	OTTO_PHYS_TO_BUS(addr,isoppo)
