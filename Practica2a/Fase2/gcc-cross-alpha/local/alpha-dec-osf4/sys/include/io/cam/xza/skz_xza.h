...
typedef enum xza_registers { 
	...
	XPD1, XPD2, ENBLUN, DISLUN, DEFLUN, TGTDIR,
	TGTIN, TGTOUT, TGTNOGO
} XZA_REGISTERS;
...
typedef struct enable_lun_reg {
	unsigned int 		rcode   : 4;	/* register emulation code */
	unsigned int            dscprv  : 1;    /* Disconnects_Mandatory */
	unsigned int            dacc    : 1;    /* Disable Auto-Check Cond. */
	unsigned int            all     : 1;    /* Disable/Default All LUNS */
	unsigned int            i       : 1;    /* Inhibit interrupt */
	unsigned int		c0	: 1;	/* channel 0 mask flag */
	unsigned int		c1	: 1;	/* channel 1 mask flag */
	unsigned int		lun     : 3;	/* Logical Unit Number */
	unsigned int		pax	: 3;	/* physical address ext */
	unsigned int		slot	: 4;	/* XMI node id of lamb */
	unsigned int		vector	: 10;	/* interrupt vector */
	unsigned int		ipl	: 2;	/* interrupt level */
} ENABLE_LUN_REG;

typedef struct target_mode_reg {
	unsigned int 		rcode   : 4;	/* register emulation code */
	unsigned int            iid     : 3;    /* Initiator ID */
	unsigned int            i       : 1;    /* Inhibit interrupt */
	unsigned int		c0	: 1;	/* channel 0 mask flag */
	unsigned int		c1	: 1;	/* channel 1 mask flag */
	unsigned int		lun     : 3;	/* Logical Unit Number */
	unsigned int		sbz	: 3;	/* Should be zero */
	unsigned int		status	: 8;	/* SCSI status */
	unsigned int		rsvrd	: 7;	/* Reserved */
	unsigned int		disc	: 1;	/* Disconnect flag */
} TARGET_MODE_REG;
...
typedef struct tgt_afar0_reg {
  unsigned int       lun:  8;
  unsigned int       iid:  8;
  unsigned int    status:  8;                   /* SCSI status */
  unsigned int     rsvrd:  6;
  unsigned int       all:  1;           /* Disable/Default Sent to all LUNS */
  unsigned int      disc:  1;           /* Disconnect MSG sent */
} TGT_AFAR0_REG;
...
#define ASRCR_EMULATION	1
#define AICR_EMULATION	2
#define CICR_EMULATION	2
#define AECR_EMULATION	3
#define CECR_EMULATION	3
#define AMCSR_EMULATION	4
#define CBRCR_EMULATION	5
#define ENBLUN_EMULATION 8
#define DISLUN_EMULATION 9  
#define DEFLUN_EMULATION 10   /* 0xA */
#define TGTDIR_EMULATION 11   /* 0xB */
#define TGTIN_EMULATION  12   /* 0xC */
#define TGTOUT_EMULATION 13   /* 0xD */
#define TGTNOGO_EMULATION 15  /* 0xE */

/* The following are used for debug */
#define XBE_EMULATION 20      /* 0x14 */
#define QIR_EMULATION 21      /* 0x15 */
#define GCQ0IR_EMULATION 22   /* 0x16 */
#define GCQ1IR_EMULATION 23   /* 0x17 */
...
#define XZA_MAX_TRANSFER	(16 * 1024 * 1024) /* 16 MB */
#define XZA_MAX_SG_ELEMENTS	1024	/* 8 bytes each, up to 8K port page */

/*
** Timing
*/
#define XZA_STATE_CHANGE_TIME	10	/* State Change wait, in seconds */
