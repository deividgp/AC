...
#else
#define Init_cbvte_lock( cbvte ) {      \
  ( cbvte )->cbvte_lk.sl_data = 0;    \
}
#define Init_pb_lock( pb ) {            \
  ( pb )->pb_lk.sl_data = 0;          \
}
#define Init_pccb_lock( pccb ) {        \
   ( pccb )->pccb_lk.sl_data = 0;     \
}
#define Init_scadb_lock() {             \
   lk_scadb.sl_data = 0;              \
}
...
#else
#define Lock_cbvte( cbvte ) {           \
    ++( cbvte )->cbvte_lk.sl_data;    \
}
#define Lock_pb( pb ) {                 \
    ++( pb )->pb_lk.sl_data;          \
}
#define Lock_pccb( pccb ) {             \
    ++( pccb )->pccb_lk.sl_data;      \
}
#define Lock_scadb() {                  \
    ++lk_scadb.sl_data;               \
}
...
#else
#define Test_pb_lock( pb )      (( pb )->pb_lk.sl_data )
#define Test_pccb_lock( pccb )  (( pccb )->pccb_lk.sl_data )
#define Test_scadb_lock()       ( lk_scadb.sl_data )
#endif
