 ...
 */
/*	
 *	@(#)$RCSfile: prestoioctl.h,v $ $Revision: 4.2.11.8 $ (DEC) $Date: 1994/05/16 23:07:21 $
 */ 
/*
 */
/*
...
/* Moved the following from presto_data.c. Now it is used in pr.c also */
...
struct prtab {
	...
	int (*pt_strategy)();		/* 8:  major dev `strategy' routine */
	int (*pt_ready)();		/* 16: major dev `ready' routine */
	int (*pt_close)();		/* 24: major dev `close' routine */
	struct prunit *pt_unit;		/* 32: enabled unit chain */
	struct prunit *pt_hunit[NPRUHASH]; /* 40: enabled unit hash chains */
};
