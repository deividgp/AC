 ...
 */
/*	
 *  @(#)$RCSfile: strkinfo.h,v $ $Revision: 4.4.13.2 $ (DEC) $Date: 1995/01/28 13:04:56 $
 */ 
/*
 */
 ...
 */
#define MAX_IPADDR_INFOS ((MAX_KINFO_BUF-sizeof(struct ipaddr_blk_hdr))/sizeof(struct ipaddr_info_blk))

struct ipaddr_blk_hdr {
       int total_cnt;           /* in/out: number info blks provided so far */
       int curr_cnt;            /* out: number of info blks in this packet */
       int more;                /* out: true if all info blks couldn't fit */
};
...
typedef struct _ipaddr_blk {
       struct ipaddr_blk_hdr  hdr;
       struct ipaddr_info_blk info[MAX_IPADDR_INFOS];
} ipaddr_blk;
