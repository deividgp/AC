 ...
 */
/*	
 *	@(#)$RCSfile: ioctl.h,v $ $Revision: 4.4.86.4 $ (DEC) $Date: 1998/10/16 15:24:22 $
 */ 
/*
...
#define	SIOCSIFTRUSTGRP	_IOW('i', 72, struct ifreq)	/* Set if_trustgrp */
#define	SIOCGIFTRUSTGRP	_IOWR('i', 73, struct ifreq)	/* Get if_trustgrp */

/*
 * #'s 74-81 should not be used in 4.0x as they already exist in 5.0.
 * You should pick #'s that are available in both to avoid conflicts.
 */

/* Network Redundant Adapters (NetRAIN) */
#define SIOCIFADD       _IOWR('i', 82, struct if_nrset) /* Attach adapter  */
#define SIOCIFSWITCH    _IOWR('i', 83, struct ifreq)    /* Switch adapters */
#define SIOCIFREMOVE    _IOWR('i', 84, struct ifreq)    /* Remove adapter */
#define SIOCIFTMOISR    _IOWR('i', 85, struct ifreq)    /* Set ISR timeout value */
#define SIOCIFTMO       _IOWR('i', 86, struct ifreq)    /* Set test timeout value */
#define SIOCIFTMOSUCCESS _IOWR('i', 87, struct ifreq)   /* Set timeout between tests */
#define SIOCIFTMORETRY  _IOWR('i', 88, struct ifreq)    /* Set timeout between tests */
#define SIOCIFTMOFAIL   _IOWR('i', 89, struct ifreq)    /* Set timeout between tests */
#define SIOCIFMAXRETRY  _IOWR('i', 90, struct ifreq)    /* Set maximum number of retries */
#define SIOCIFMONITOR   _IOWR('i', 91, struct ifreq)    /* Enable monitoring of interface */
#define SIOCGNRSET      _IOWR('i', 92, struct if_nrset) /* Get list of netrain members */
#define SIOCIFAUTOFAIL  _IOWR('i', 93, struct ifreq)    /* Enable auto failover mode */
#define SIOCIFTMODOWN   _IOWR('i', 94, struct ifreq)    /* Set timeout between tests */

/*
 * #'s 95-99 are avaliable for use as they are currently unused in 5.0.
 *
 * #'s 100-106 should not be used in 4.0x as they already exist in 5.0.
 * You should pick #'s that are available in both to avoid conflicts.
 */
