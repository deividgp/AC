 ...
 */
/*
 *	@(#)$RCSfile: server_loop.c,v $ $Revision: 4.3.5.2 $ (DEC) $Date: 1993/05/26 22:08:17 $
 */
/*
	...
	boolean_t	do_notify;
{
	...
	for (;;) {
		...
		if (SERVER_DISPATCH(in_msg, out_msg) &&
			...
			register port_t dest_port = out_msg->msg_remote_port;  

			msg_send(out_msg, MSG_OPTION_NONE, 0);
			port_deallocate(my_self, dest_port);
		}
	}
}
